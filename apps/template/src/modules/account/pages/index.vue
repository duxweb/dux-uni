...
import type { UniAuthState } from '@duxweb/uni'
import { useAuth, useCheck, useLogout } from '@duxweb/uni'

const auth = useAuth()
const authState = computed<UniAuthState | null>(() => (auth.value || null) as UniAuthState | null)
const isLogin = computed(() => Boolean(authState.value?.token))
const userName = computed(() => String(authState.value?.user?.name || '未设置'))
const tokenText = computed(() => String(authState.value?.token || '无'))
const permissions = computed(() => {
  const value = authState.value?.permissions
  ...
  <view class="flex flex-col gap-[24rpx]">
    <view class="rounded-card bg-surface p-page shadow-[0_12rpx_32rpx_rgba(15,23,42,0.06)]">
      ...
      <wd-cell-group border custom-class="rounded-[24rpx]! overflow-hidden!">
        ...
        <wd-cell title="用户名称" :value="userName" />
        <wd-cell title="Token" :value="tokenText" />
      </wd-cell-group>
    </view>
    ...
    <view class="grid grid-cols-2 gap-[16rpx]">
      <wd-button plain :loading="checkAction.isPending.value" @click="validateSession">
        校验会话
      </wd-button>
      <wd-button type="primary" :loading="logoutAction.isPending.value" @click="logoutSession">
        退出登录
      </wd-button>
    </view>
  </view>
