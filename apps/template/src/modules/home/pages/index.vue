...
<template>
  <view class="flex flex-col gap-[24rpx]">
    ...
    <view class="rounded-card bg-surface p-page shadow-[0_12rpx_32rpx_rgba(15,23,42,0.06)]">
      <view class="mb-gap flex flex-col gap-[8rpx]">
        <text class="text-text text-[30rpx] font-semibold">业务状态持久化</text>
        <text class="text-text-secondary text-[24rpx] leading-relaxed">
          模板已接入 `pinia-plugin-persist-uni`。业务 store 可以直接通过 `persist` 开启本地持久化，认证会话仍由 `dux-uni` 自己管理。
        </text>
      </view>
      <view class="flex flex-col gap-[16rpx]">
        <wd-tag plain type="primary">
          模板说明：{{ preferencesStore.introCollapsed ? '已折叠' : '展开中' }}
        </wd-tag>
        <wd-button plain block @click="preferencesStore.toggleIntro()">
          {{ preferencesStore.introCollapsed ? '重新展开模板说明' : '折叠模板说明' }}
        </wd-button>
      </view>
    </view>
